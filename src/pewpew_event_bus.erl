...
trigger(EventMgr, EventName) ->
  trigger(EventMgr, {EventName, pewpew_event_bus_no_params}).
trigger(EventMgr, EventName, Data) ->
  gen_event:notify(EventMgr, {event, EventName, params, Data}).

init(_) ->
  {ok, pewpew_event_bus_data:new([])}.

handle_call({on, EventName, EventCallback}, EventBusData) ->
  Events = pewpew_event_bus_data:events(EventBusData),
  {ok, cona, pewpew_event_bus_data:update(EventBusData, [{events, [{EventName, EventCallback} | Events]}])}.

handle_event({event, Event, params, pewpew_event_bus_no_params}, EventBusData) ->
...
event_callbacks(EventBusData, Event) ->
  [Callback || {_, Callback} <- pewpew_event_bus_data:events(EventBusData)].
